...
def show_samples_generic(test_ids, namespace):
    ...
    try:
        show_all_sensors = namespace.checkbox(
            "Show All Sensors",
            value=st.session_state["show_all_sensors"],
            key="show_all_sensors",
        )
    except KeyError:
        show_all_sensors = namespace.checkbox(
            "Show All Sensors", value=True, key="show_all_sensors"
        )
    ...
    try:
        sensor_mask = namespace.multiselect(
            "Sensors",
            sensors,
            default=st.session_state["sensor_mask"],
            key="sensor_mask",
        )
    except KeyError:
        sensor_mask = namespace.multiselect(
            "Sensors", sensors, default=sensors, key="sensor_mask"
        )
